...
double maspoblado(int *proporciones, int tamanio){
	double bicho = 0;
...
int main(){
	...
	int q=10; //numero de estados posibles para cada sitio (valor minimo: 2) TOMAR 2,4,10
	...
	*/
	...
	pointer_medidas= fopen("medidas_q10_L20.txt", "w");
	pointer_medidasE= fopen("datosE_q10_L20.txt", "w");
	int datos=301;
	for(int xx=1; xx<datos; xx++){
		...
		for(int pasos=0; pasos<MCS; pasos++){
			...
			if (pasos >= (pasos_termalizacion - 1)){
				...
				max_fraction += maspoblado(subtotales,q);
				cv += pow(EEE,2);
